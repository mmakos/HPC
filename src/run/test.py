...
def readDataset( dsName ):
    with np.load( "../../dataFinal/datasets/" + dsName, allow_pickle=True ) as data:
        img = data[ 'images' ]
        lab = data[ 'labels' ]
...
def getModel( modelName ):
    mod = tf.keras.models.load_model( '../../dataFinal/models/' + modelName )
...
plt.figure()
sns.heatmap( confusionMatrix, xticklabels=poses, yticklabels=poses, annot=True, fmt='.2g', cmap="YlOrBr" )
